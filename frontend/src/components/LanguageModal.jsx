    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex
      ...
      <div className="w-[50%] h-[80%] bg-white p-2 rounded-xl ">
        <div className="bg-white flex items-center justify-start p-5">
            <MdClose onClick={onClose} className="bg-white text-3xl cursor-pointer hover:bg-slate-100 p-1 rounded-full" />
        </div>

        <div className="bg-white flex items-center justify-start gap-5 p-5">
            <p onClick={handleBorderState1}
             className={`bg-white cursor-pointer ${showBorder1 ? 'border-b-2 border-blue-400' : 'border-b-0'}`}>Language</p>
            <p onClick={handleBorderState2}
             className={`bg-white cursor-pointer ${showBorder2 ? 'border-b-2 border-blue-400' : 'border-b-0'}`}>Currency</p>
        </div>
        <div className="w-full bg-white p-5">
            <div className="bg-white w-full grid grid-rows-4 grid-flow-col gap-4">
                {
                    languages.map((lg => (
                        <div onClick={()=>handleLanguageClick(lg.title)}
                         className={`rounded-xl p-3 cursor-pointer$ ${selectedLanguage === lg.title ? 'border border-blue-300' : ''}`}>
                            <h1>{lg.title}</h1>
                            <p>{lg.subtitle}</p>
                        </div>
                    )
